...
#---------browser-functions------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    if True:
        ...
        def add_bookmark(self):
            ...
            if len(c.fetchall()) < 1:
                c.execute(f"INSERT INTO bookmarks VALUES ('{url}','{self.tabs.currentWidget().page().title().replace('Free Hentai Manga, Doujinshi and Anime Porn','Home').replace(' - HentaiFox','').replace(' - Hentai Galleries','').replace('Free Hentai Manga and Doujinshi','Overview')}')")
                conn.commit()
        ...
        def refresh_bookmarks(self,mode=None):
            ...
            if mode == "remove":
                ...
                for bookmark in bookmarks:
                    self.bookmarkMenu.addAction(f"{bookmark[1]}",lambda bookmark_url=bookmark[0]: self.load_bookmark(bookmark_url))
            elif mode == "add":
        ...
        def multi_search(self):
            if self.whitelist.count() > 0:
                ...
                if len(results) > 0:
                    ...
                    self.label.setText(f'Found {len(results)} galleries. You can find "{filename}.txt" in the "Results (TXT)" folder and "{filename}.result" in the "Results (JSON)" folder.')
                else:
    ...
    if True:
        ...
        def display_results(self):
            ...
            if len(result_view_list) > 0:
                c.execute(f"SELECT * FROM galleryinformation WHERE gal IN {tuple(result_view_list)} ORDER BY {feature} {up_down}")
